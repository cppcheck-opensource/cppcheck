...
#define instantiateMatch(...) instantiateMatch_(__FILE__, __LINE__, __VA_ARGS__)
    ...
    { // a non template overload might be a better match: don't deduce
        {
            // the non template h(char) wins the overload resolution
            const char code[] = "template<class T> T h(T t) { return t; }\n"
                                "long h(char);\n"
                                "void use1() {\n"
                                "    char c = 'x';\n"
                                "    h(c);\n"
                                "}";
            const char exp[] = "template < class T > T h ( T t ) { return t ; } "
                               "long h ( char ) ; "
                               "void use1 ( ) { "
                               "char c ; c = 'x' ; "
                               "h ( c ) ; "
        ...
        {
            // non template overload in a base class
            const char code[] = "struct Base {\n"
                                "    template<class T> void bf(T t) { (void)t; }\n"
                                "    void bf(int);\n"
                                "};\n"
                                "struct Derived : public Base {\n"
        ...
        {
            // overloaded function templates => don't deduce
            const char code[] = "template<class T> void g(T t) { (void)t; }\n"
                                "template<class T> void g(T* t) { (void)t; }\n"
                                "void use(long l) { g(l); }";
